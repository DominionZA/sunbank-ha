...
shows the link's health back in HA. It authenticates with the Home Assistant source **Push key**
from Sunbank's Integrations page and pushes readings mapped to the published metric catalog.

## What it does (v0)
- On an interval (default 60s), reads a set of HA sensors, maps them to Sunbank metric keys, and
  `POST`s them to `/v1/ingest` with `Authorization: Bearer <push_key>`.
...
4. Enter:
   ...
   - **Push key** — from the Sunbank dashboard → Integrations → Home Assistant integration → Push key.
